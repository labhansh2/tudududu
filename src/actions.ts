...
export async function toggleWorkingStatus(taskId: number, activeTask : Task | undefined, timezone: string) {
...
export async function updateWorkTime(taskId: number, endedAt: Date, timezone: string) {
  ...
  for (const [dateStr, seconds] of timeMap.entries()) {
    ...
    const existing = await db.query.workTime.findFirst({
      where: and(
        eq(workTime.userId, task.userId),
        eq(workTime.date, date)
      ),
    });
